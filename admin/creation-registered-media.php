...
add_action( 'admin_menu', 'wpclink_register_creations_media_menu' );
/**
 * CLink Register Creation Media Menu
 * 
 */
function wpclink_register_creations_media_menu() {
	...
	if ( wpclink_user_has_creator_list($current_user_id) ) {
		...
		if ( wpclink_export_mode() || wpclink_both_mode() ) {
			add_submenu_page( 'cl_mainpage.php', 'Creations', 'Creations', 'edit_posts', 'wpclink-creation-media', 'wpclink_display_creations_media' );
		}
		...
		if ( wpclink_export_mode() || wpclink_both_mode() ) {
			add_submenu_page( 'cl_mainpage.php', 'Creation', 'Creation', 'manage_options', 'wpclink-creation-media', 'wpclink_display_creations_media' );
		}
	}
}
/**
 ...
 * 
 */
function wpclink_display_creations_media() {
	...
	<div id="cl_main" class="wrap all_list_post">
		...
		<!-- Start tabs -->
		<ul class="wp-tab-bar">
			<li><a href="<?php echo menu_page_url( 'cl-restriction', false ); ?>">Posts</a>
			</li>
			...
			<li><a href="<?php echo menu_page_url( 'cl-restriction-page-available', false ); ?>">Pages</a>
			</li>
		...
		<div class="wp-tab-panel" id="tabs-1">
			...
			$current_page = menu_page_url( 'wpclink-creation-media', false );
	...
	$license_class = get_post_meta($post_id,'wpclink_creation_license_class',true); ?>
						<tr id="post-<?php echo $post_id; ?>" class="iedit referent_list author-self level-0 post-<?php echo $post_id; ?> type-post status-draft format-standard hentry category-uncategorized">
							...
							<td class="title column-title has-row-actions column-primary page-title" data-colname="Title">
								...
								<div class="row-actions"><span class="edit"><a href="<?php echo 'post.php?post='.$post_id.'&action=edit'; ?>" aria-label="Edit “(no title)”">Edit</a> | </span><span class="view"><a href="<?php echo get_permalink($post_id) ?>" rel="bookmark" aria-label="View">View</a></span>
								</div>
